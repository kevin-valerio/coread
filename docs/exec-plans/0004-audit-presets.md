...
When the user validates a codebase path, the app starts both preset analyses in the background if that path does not already have cached markdown in localStorage. Each preset runs Codex with `gpt-5.5`, high reasoning, and read-only sandboxing. The UI stores the final markdown per codebase path and preset. A refresh control reruns the selected preset and replaces the cached markdown.
